...
public class WeatherContentProvider extends ContentProvider {
	...
	public static final Uri CONTENT_CURRWEATHER_URI = Uri.parse("content://"
			+ AUTHORITY + "/weather_cities");

	private static final UriMatcher uriMatcher;
	static {
		uriMatcher = new UriMatcher(UriMatcher.NO_MATCH);
		uriMatcher.addURI(AUTHORITY, "weather_cities", ALL_CURRWEATHER_CITIES);
		uriMatcher.addURI(AUTHORITY, "weather_cities/#",
				SINGLE_CURRWEATHER_CITY);
	...
	public Uri insert(Uri uri, ContentValues values) {
		...
		case ALL_CURRWEATHER_CITIES:
			// Using insertWithOnConflict with conflict method
			// "CONFLICT_REPLACE" so that if there is
			// a conflict addition for same city then conflicting data is
			// replaced with latest one

			id = db.insertWithOnConflict(
					CurrentWeatherDb.CURRENT_WEATHER_TABLE, null, values,
	...
	public int delete(Uri uri, String selection, String[] selectionArgs) {
		...
		deleteCount = db.delete(CurrentWeatherDb.CURRENT_WEATHER_TABLE,
